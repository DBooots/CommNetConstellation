...
namespace CommNetConstellation.CommNetLayer
{
    ...
    public class CNCCommNetwork : CommNetwork
    {
        ...
        protected override bool SetNodeConnection(CommNode a, CommNode b)
        {
            ...
            try
            {
                aFreq = (a.isHome) ? publicFreq : ((IModularCommNetVessel)a.GetVessel().Connection).GetModuleOfType<CNCCommNetVessel>().getRadioFrequency();//((CNCCommNetVessel)CNCCommNetScenario.Instance.findCorrespondingVessel(a).Connection).getRadioFrequency();
                bFreq = (b.isHome) ? publicFreq : ((IModularCommNetVessel)b.GetVessel().Connection).GetModuleOfType<CNCCommNetVessel>().getRadioFrequency();//((CNCCommNetVessel)CNCCommNetScenario.Instance.findCorrespondingVessel(b).Connection).getRadioFrequency();
            }
            ...
            try
            {
                aMembershipFlag = (a.isHome) ? false : ((IModularCommNetVessel)a.GetVessel().Connection).GetModuleOfType<CNCCommNetVessel>().getMembershipFlag();
                bMembershipFlag = (b.isHome) ? false : ((IModularCommNetVessel)b.GetVessel().Connection).GetModuleOfType<CNCCommNetVessel>().getMembershipFlag();
            }
            catch(NullReferenceException e) // either CommNode could be a kerbal on EVA
            {
                this.Disconnect(a, b, true);
